...
Phase 2 adds: an authenticated Help Page; Excel Trip Import with pre-flight validation; chronological Page renumbering on retroactive insertion with re-editable Book Opening; Estimated Start Time (`end − distance/20` ceiled to 5 min) auto-suggest plus manual override and optional Start Time; Landing login page with Super Admin (Neranjan) forced password change and Gmail allow-list-gated Google SSO; "Trip Added" toast + redirect to Dashboard; Transposed Side 2 tables mirroring the physical book layout; revised fuel arithmetic `Closing = Position + In-Tank + Drawn − Consumed` with In-Tank default 0; Integer KM (no decimals) for odometer fields with fuel at 1 decimal; clear-font spacious ledger tables; Dashboard This-Month Official/Private/Total; Vehicle Registration No and removal of current fuel level from profile; Global Search; last-12 Months/Pages summaries with More modal; and Continuity Break alerts with optional recalculation.
...
24. As a driver, I want Start Time auto-suggested as `End Time − (Trip Distance / 20 km/h)`, ceiled to the nearest 5 minutes, when Distance and End Time are present but Start Time is empty, so that I don't have to estimate manually.
...
29. As a visitor, I want a Landing login page as the only view when unauthenticated, showing Super Admin password login and Google SSO, so that ledger data is not exposed.
30. As Neranjan (Super Admin), I want to sign in with username Neranjan and password `SupAd@2000` (hashed), and be forced to change the password on first login, so that the bootstrap credential is not reused.
31. As Super Admin, I want to define the Gmail allow-list (one or many addresses) that may sign in via Google SSO, so that access is explicitly gated.
32. As an Allowed Email user, I want to sign in via Google SSO and then see Dashboard, Ledger, Trips, and Help, so that authorized drivers can work.
33. As a non-allowed Gmail user, I want Google SSO to reject me with "Not authorized — contact admin" and keep me on Landing, and as any unauthenticated user I must see only Landing (and Help after login per grilling), so that data isolation holds.
...
- **Auth & Allow-List:** Supabase auth extended with Super Admin bootstrap (hashed `SupAd@2000`, forced change redirect) and Gmail allow-list CRUD only for Super Admin; Google SSO callback checks allow-list, rejecting non-listed users with Landing-only view (see ADR 0004).
- **Ledger & Dashboard Presentation:** Side 2 tables Transposed (days as columns) matching Side 1 orientation, with optional toggle; Dashboard computes This-Month trio via filtered date prefix `YYYY-MM`; monthly and Page-wise breakdowns default to last 12 with More modal (virtualized scroll, sticky header, monospace ints); Global Search reuses the existing master-table filter as a header live debounced query with summed distances footer; continuity alerts surface `validateOdometerContinuity`/`validateFuelContinuity` breaks with recalc/renumber actions behind confirmation.
...
- **Which modules will be tested:**
  - Trip & time reciprocals and Estimated Start Time ceiling (distance-first, 20 km/h, ceil 5 min, optional Start Time, End Time default now).
  ...
  - Dashboard This-Month metrics (integer KM), monthly/Page-wise last-12 + More modal data, and Global Search live filtering with summed distances.
  - Auth allow-list gating (Super Admin bootstrap + forced change, Gmail allow-list CRUD, Google SSO accept/reject, Landing-only for non-allowed), Help Page visibility, and "Trip Added" toast + redirect.
...
- Real-time GPS speed adaptation for time estimation (fixed 20 km/h) and reserved-gap page numbering (configurable print offset may be added later).
- Full i18n/localization and role-based row-level policies beyond Super Admin / Allowed Email.

## Further Notes

- Domain glossary lives in `CONTEXT.md` (Book, Page, Trip, Vehicle, Book Opening, Fuel Economy/Position/In-Tank/Drawn/Consumed/Closing Balance, Integer KM, Super Admin, Allowed Email, Landing, Help Page, Trip Import, Estimated Start Time, Global Search, Continuity Break, Transposed Side 2) — spec uses those exact terms.
- Decisions hard to reverse are recorded in `docs/adr/0001-chronological-page-renumbering`, `0002-integer-km-fuel-decimals`, `0003-fuel-formula-with-in-tank`, `0004-super-admin-gated-google-sso`.
